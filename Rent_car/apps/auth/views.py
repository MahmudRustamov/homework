...
def register():
    ...
    for mail in users:
        if email == mail[2]:
            ...
            break
    if is_found:
        print("This account is already registered. Enter another email!!")
        return register()   
    else:
        password1 = input("Enter a new password: ")
        password2 = input("Confirm your password again: ")
...
def show_users():
    ...
    if users:
        ...
        for user in users:
            try:
                print(f"ID: {user[0]}  Full Name: {user[1]}  Email: {user[2]}  Date: {user[-1].split(" ")[0]}")
            except IndexError:
                print("Warning: Some user data is incomplete.")
    else:
